...
public class View {
    ...
    protected void runMove() { // TODO
    	if(this.model.getCurrentAgent() instanceof HumanAgent) {
    		Move move = new Move(this.boardPanel.getpieceselected(),this.boardPanel.getsecondpieceselected());
    		this.model.move(move);
    		this.boardPanel.setpieceselected(null);
    		this.boardPanel.setsecondpieceselected(null);
    		this.setUndoButton();
    		this.boardPanel.updateCells();
    		if(this.model.getBoard().isGameOver()) {
        		this.setMessageLabel(this.model.getBoard().getWinner().getType());
        		for(int i = 0; i < this.model.getBoard().board.length;i++) {
            		for(int j = 0; j < this.model.getBoard().board[i].length;j++) {
            			this.model.getBoard().board[i][j].setDisable(true);
                	}
            	}
        	}
    		this.setMessageLabel(this.model.getBoard().getTurn().getType());
    	}
    	else if(this.model.getCurrentAgent() instanceof GreedyAgent || this.model.getCurrentAgent() instanceof RandomAgent) {
    		Move move2 = this.model.getCurrentAgent().getMove();
    		this.model.move(move2);
    		this.setUndoButton();
    		this.boardPanel.updateCells();
    		this.setMessageLabel(this.model.getBoard().getTurn().getType());
    		if(this.model.getBoard().isGameOver()) {
...
//    	if ((model.getCurrentAgent() instanceof HumanAgent) && this.model.getMovesSize() == 1 && this.gameMode != ThreeMusketeers.GameMode.Human) {
//    		this.undoButton.setDisable(false);
//    		
//   	}
    	 if(this.model.getMovesSize()>0) {
    		this.undoButton.setDisable(false);
    	}else if(this.model.getMovesSize() == 0) {
    ...
    protected void setGameMode(ThreeMusketeers.GameMode mode) { // TODO
    	this.gameMode = mode;
    	if(this.gameMode == ThreeMusketeers.GameMode.Human) {
    		this.model.selectMode(gameMode, Piece.Type.MUSKETEER);
    		this.gameModeLabel.setText(mode.getGameModeLabel());
    		this.showBoard();
    		
    	}else if(this.gameMode == ThreeMusketeers.GameMode.HumanRandom || this.gameMode == ThreeMusketeers.GameMode.HumanGreedy){
    		this.gameModeLabel.setText(mode.getGameModeLabel());
    ...
    private void saveBoard() { // TODO
    	File file = new File("boards/" + saveFileNameTextField.getText());
    	if(!(saveFileNameTextField.getText().endsWith(".txt"))) {
