...
public class ModeInputPanel extends GridPane {
    ...
    private int getFiles(ListView<String> listView) { // TODO
    	String path = "boards";
    	ObservableList<String> filesinpath =FXCollections.observableArrayList();
		File folder = new File(path);
		for(File file:folder.listFiles()) {
			if(file.getName().endsWith(".txt")) {
				filesinpath.add(file.getName());
			}
		}
    	listView.setItems(filesinpath);
    ...
    private void selectBoard(Label selectBoardLabel, ListView<String> boardsList) { // TODO
    	...
    	File boardSelected = new File("boards/" + selectedItem);
    	this.view.model.setBoard(boardSelected);
    	selectBoardLabel.setText("Current board: " + selectedItem);
    }
}
